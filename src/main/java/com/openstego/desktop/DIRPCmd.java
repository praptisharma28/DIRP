...
import com.DIRP.desktop.util.cmd.CmdLineOption;
import com.DIRP.desktop.util.cmd.CmdLineOptions;
import com.DIRP.desktop.util.cmd.CmdLineParser;
import com.DIRP.desktop.util.cmd.PasswordInput;

import java.io.File;
import java.util.List;

/**
 * This is the main class for DIRP command line
 */
public class DIRPCmd {
    /**
    ...
    public static void execute(String[] args) {
        ...
        try {
            ...
            if (pluginName != null && !pluginName.equals("")) {
                ...
                if (plugin == null) {
                    throw new DIRPException(null, DIRP.NAMESPACE, DIRPErrors.PLUGIN_NOT_FOUND, pluginName);
                }
            }
            // Try to auto-select plugin
            else {
                List<DIRPPlugin<?>> plugins = PluginManager.getPlugins();
                if (plugins.size() == 1) {
            ...
            if (!command.equals("help") && !command.equals("algorithms")) {
                if (plugin == null) {
                    throw new DIRPException(null, DIRP.NAMESPACE, DIRPErrors.NO_PLUGIN_SPECIFIED);
                } else {
                    // Create main stego object
                    plugin.resetConfig(parser.getParsedOptions());
                    stego = new DIRP(plugin, plugin.getConfig());
                }
            }
        ...
        } catch (DIRPBulkException bulkEx) {
     ...
     * @param stego   {@link DIRP} object
     * @throws DIRPException     Processing issues
     * @throws DIRPBulkException Errors for multiple files
     */
    private static void executeEmbed(CmdLineOptions options, DIRP stego)
            throws DIRPException, DIRPBulkException {
     ...
     * @throws DIRPBulkException Errors for multiple files
     */
    private static void executeEmbedMark(CmdLineOptions options, DIRP stego)
            throws DIRPException, DIRPBulkException {
     ...
     * @throws DIRPException Processing issues
     */
    private static void executeExtract(CmdLineOptions options, DIRP stego) throws DIRPException {
        String stegoFileName = options.getStringValue("-sf");
        ...
        try {
            ...
            if (osEx.getErrorCode() == DIRPErrors.INVALID_PASSWORD || osEx.getErrorCode() == DIRPErrors.NO_VALID_PLUGIN) {
                if (stego.getConfig().getPassword() == null) {
                    stego.getConfig().setPassword(PasswordInput.readPassword(labelUtil.getString("cmd.msg.enterPassword") + " "));

                    try {
                        msgData = stego.extractData(new File(stegoFileName));
                    } catch (DIRPException inEx) {
                        if (inEx.getErrorCode() == DIRPErrors.INVALID_PASSWORD) {
     ...
     * @throws DIRPException Processing issues
     */
    private static void executeCheckMark(CmdLineOptions options, DIRP stego) throws DIRPException {
        String stegoFileName = options.getStringValue("-sf");
        String sigFileName = options.getStringValue("-gf");
     ...
     * @throws DIRPException Processing issues
     */
    private static void executeGenSig(CmdLineOptions options, DIRP stego) throws DIRPException {
        // Check if we need to prompt for password
        if (stego.getConfig().getPassword() == null) {
            stego.getConfig().setPassword(PasswordInput.readPassword(labelUtil.getString("cmd.msg.enterPassword") + " "));
        }
     ...
     * @throws DIRPException Processing issues
     */
    private static void executeDiff(CmdLineOptions options, DIRP stego) throws DIRPException {
        String coverFileName = options.getStringValue("-cf");
        String stegoFileName = options.getStringValue("-sf");
        String extractDir = options.getStringValue("-xd");
    ...
    /**
     * Method to display usage for DIRP
     *
     * @throws DIRPException Processing issues
     */
    private static void displayUsage() throws DIRPException {
     ...
     * @throws DIRPException Processing issues
     */
    private static CmdLineOptions getStdCmdLineOptions(DIRPPlugin<?> plugin) throws DIRPException {
