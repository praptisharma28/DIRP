...
/**
 * This is the class for providing cryptography support to DIRP.
 */
public class DIRPCrypto {
    /**
     ...
     */
    public DIRPCrypto(String password, String algorithm) throws DIRPException {
        KeySpec keySpec;
        ...
        try {
            if (algorithm == null || algorithm.trim().equals("") || ALGO_AES128.equalsIgnoreCase(algorithm)) {
                ...
                throw new DIRPException(null, DIRP.NAMESPACE, DIRPErrors.INVALID_CRYPT_ALGO, algorithm);
            }
     ...
     * @throws DIRPException Processing issues
     */
    public byte[] encrypt(byte[] input) throws DIRPException {
        try {
     ...
     * @throws DIRPException Processing issues
     */
    public byte[] decrypt(byte[] input) throws DIRPException {
        try {
            ...
            throw new DIRPException(bpEx, DIRP.NAMESPACE, DIRPErrors.INVALID_PASSWORD);
        } catch (Exception ex) {
            throw new DIRPException(ex);
        }
    }
