...
import com.DIRP.desktop.plugin.template.dct.DCTConfig;
import com.DIRP.desktop.plugin.template.dct.DCTDataHeader;
...
import com.DIRP.desktop.util.dct.DCT;

import java.io.IOException;
import java.util.List;

/**
 * Plugin for DIRP which implements the DCT based Least-significant bit algorithm
 */
public class DctLSBPlugin extends WMImagePluginTemplate {
    ...
    public byte[] extractData(byte[] stegoData, String stegoFileName, byte[] origSigData) throws DIRPException {
        ...
        try (DctLSBInputStream is = new DctLSBInputStream(imgHolder, this.config)) {
            ...
            if (bytesRead != msg.length) {
                throw new DIRPException(null, NAMESPACE, DctLSBErrors.ERR_IMAGE_DATA_READ);
            }
        } catch (IOException ioEx) {
            throw new DIRPException(ioEx);
        }
     ...
     */
    @Override
    protected DCTConfig createConfig(CmdLineOptions options) throws DIRPException {
