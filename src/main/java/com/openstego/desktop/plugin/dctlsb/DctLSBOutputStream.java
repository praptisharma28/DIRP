...
public class DctLSBOutputStream extends OutputStream {
     ...
     */
    public DctLSBOutputStream(ImageHolder image, int dataLength, String fileName, DIRPConfig config) throws DIRPException {
        List<int[][]> yuv;
     ...
     *
     * @throws DIRPException Writing issues
     */
    private void writeHeader() throws DIRPException {
        ...
        try {
            ...
            if (((header.getHeaderSize() + this.dataLength) * 8) > (this.imgWidth * this.imgHeight / (DCT.NJPEG * DCT.NJPEG))) {
                throw new DIRPException(null, DctLSBPlugin.NAMESPACE, DctLSBErrors.IMAGE_SIZE_INSUFFICIENT);
            }
     ...
     * @throws DIRPException Processing issues
     */
    public ImageHolder getImage(int imgType) throws DIRPException {
        try {
            flush();
        } catch (IOException ioEx) {
            throw new DIRPException(ioEx);
