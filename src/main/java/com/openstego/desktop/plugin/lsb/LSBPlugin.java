...
import com.DIRP.desktop.plugin.template.image.DHImagePluginTemplate;
...
/**
 * Plugin for DIRP which implements the Least-significant bit algorithm of steganography
 */
public class LSBPlugin extends DHImagePluginTemplate<LSBConfig> {
     ...
     */
    @Override
    public String extractMsgFileName(byte[] stegoData, String stegoFileName) throws DIRPException {
    ...
    public byte[] extractData(byte[] stegoData, String stegoFileName, byte[] origSigData) throws DIRPException {
        ...
        try (LSBInputStream lsbIS = new LSBInputStream(imgHolder, this.config)) {
            ...
            if (bytesRead != data.length) {
                throw new DIRPException(null, NAMESPACE, LSBErrors.ERR_IMAGE_DATA_READ);
            }
     ...
     */
    @Override
    protected LSBConfig createConfig(CmdLineOptions options) throws DIRPException {
