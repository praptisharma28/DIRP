...
package com.DIRP.desktop.plugin.randlsb;

import com.DIRP.desktop.DIRPConfig;
import com.DIRP.desktop.DIRPException;
import com.DIRP.desktop.plugin.lsb.LSBConfig;
import com.DIRP.desktop.plugin.lsb.LSBDataHeader;
import com.DIRP.desktop.plugin.lsb.LSBErrors;
import com.DIRP.desktop.plugin.lsb.LSBPlugin;
...
public class RandomLSBOutputStream extends OutputStream {
     ...
     */
    public RandomLSBOutputStream(ImageHolder image, int dataLength, String fileName, DIRPConfig config) throws DIRPException {
        if (image == null || image.getImage() == null) {
            throw new DIRPException(null, LSBPlugin.NAMESPACE, LSBErrors.NULL_IMAGE_ARGUMENT);
        }
     ...
     * @throws DIRPException Processing issues
     */
    private void writeHeader() throws DIRPException {
        int channelBits = 1;
        int noOfPixels;
        int headerSize;
            ...
            while (true) {
                if ((noOfPixels * 3 * channelBits) / 8.0 < (headerSize + this.dataLength)) {
                    ...
                    if (channelBits > ((LSBConfig) this.config).getMaxBitsUsedPerChannel()) {
                        throw new DIRPException(null, LSBPlugin.NAMESPACE, LSBErrors.IMAGE_SIZE_INSUFFICIENT);
                    }
                } else {
                    break;
                }
