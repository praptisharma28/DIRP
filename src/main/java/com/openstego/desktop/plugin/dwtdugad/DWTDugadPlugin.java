...
import com.DIRP.desktop.plugin.template.image.WMImagePluginTemplate;
import com.DIRP.desktop.util.ImageHolder;
import com.DIRP.desktop.util.ImageUtil;
import com.DIRP.desktop.util.LabelUtil;
import com.DIRP.desktop.util.StringUtil;
import com.DIRP.desktop.util.dwt.DWT;
import com.DIRP.desktop.util.dwt.Image;
import com.DIRP.desktop.util.dwt.ImageTree;
...
/**
 * Plugin for DIRP which implements the DWT based algorithm by Dugad.
 * <p>
...
public class DWTDugadPlugin extends WMImagePluginTemplate {
    ...
    public byte[] embedData(byte[] msg, String msgFileName, byte[] cover, String coverFileName, String stegoFileName) throws DIRPException {
        ...
        if (cover == null) {
            throw new DIRPException(null, NAMESPACE, DWTDugadErrors.ERR_NO_COVER_FILE);
        } else {
        ...
        for (int i = 0; i < sig.decompositionLevel; i++) {
            if (s.getHorizontal() == null || s.getVertical() == null || s.getDiagonal() == null) {
                throw new DIRPException(null, NAMESPACE, DWTDugadErrors.ERR_FILE_TOO_SMALL);
            }
     ...
     */
    @Override
    public byte[] extractData(byte[] stegoData, String stegoFileName, byte[] origSigData) throws DIRPException {
        ImageHolder image;
        DWT dwt;
        ImageTree dwtTree;
        ImageTree s;
     ...
     */
    @Override
    public double getWatermarkCorrelation(byte[] origSigData, byte[] watermarkData) throws DIRPException {
        ...
        try {
            ...
            if (n == -1 || !WM_MARKER.equals(new String(markArr))) {
                throw new DIRPException(null, NAMESPACE, DWTDugadErrors.ERR_SIG_NOT_VALID);
            }
    ...
    private static class Signature {
         ...
         * @throws DIRPException Processing issues
         */
        public Signature(byte[] sigData) throws DIRPException {
            ObjectInputStream ois;
            int n;
         ...
         * @throws DIRPException Processing issues
         */
        public byte[] getSigData() throws DIRPException {
            try (
                    ByteArrayOutputStream baos = new ByteArrayOutputStream();
                    ObjectOutputStream oos = new ObjectOutputStream(baos)
            ) {
