...
public class DCTDataHeader {
     ...
     */
    public DCTDataHeader(int dataLength, String fileName, DIRPConfig config) {
     ...
     */
    public DCTDataHeader(InputStream dataInStream, DIRPConfig config) throws DIRPException {
        int stampLen;
        int versionLen;
        int fileNameLen;
        int n;
        ...
        try {
            ...
            if (n == -1 || !(new String(stamp)).equals(new String(DATA_STAMP))) {
                throw new DIRPException(null, DCTPluginTemplate.NAMESPACE, DCTErrors.INVALID_STEGO_HEADER);
            }

            n = dataInStream.read(version, 0, versionLen);
            if (n == -1 || !(new String(version)).equals(new String(HEADER_VERSION))) {
                throw new DIRPException(null, DCTPluginTemplate.NAMESPACE, DCTErrors.INVALID_HEADER_VERSION);
            }
